...
export class ChannelStartupService {
  ...
  public async fetchChats(query: any) {
    ...
    // - The API DTO (Query<>) uses page/offset.
    // - Some clients use take/skip.
    // Normalize to take/skip so LIMIT/OFFSET work predictably.
    const take = Number(query?.take ?? query?.offset ?? 50);
    const page = Number(query?.page ?? 1);
    const normalizedTake = Number.isFinite(take) && take > 0 ? take : 50;
    const normalizedSkip = Number(
      query?.skip ?? normalizedTake * (page === 1 ? 0 : Math.max(page - 1, 0)),
    );
    ...
    const limit = Prisma.sql`LIMIT ${normalizedTake}`;
    const offset = normalizedSkip ? Prisma.sql`OFFSET ${normalizedSkip}` : Prisma.sql``;
    ...
    const results = await this.prismaRepository.$queryRaw`
      WITH rankedMessages AS (
        SELECT DISTINCT ON ("Message"."key"->>'remoteJid') 
          ...
          -- Keep chat name separate to avoid overriding the computed pushName above.
